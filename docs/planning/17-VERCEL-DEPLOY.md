...
API / DB / Lemon Squeezy / email (server):
...
Create the Lemon Squeezy webhook at
`https://YOUR-DOMAIN/v1/billing/webhook`. Enable `order_created`,
`order_refunded`, `subscription_created`, `subscription_updated`,
`subscription_cancelled`, `subscription_resumed`, `subscription_expired`,
`subscription_paused`, and `subscription_unpaused`. Use the same signing
secret in `LEMON_SQUEEZY_WEBHOOK_SECRET`.
